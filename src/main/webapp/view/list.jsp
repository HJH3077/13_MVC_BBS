			...
			<tbody>
				<c:choose>
					<c:when test="${empty list}">
						<tr>
							<td colspan="5"><h2>정보가 존재하지 않습니다.</h2></td>
						</tr>
					</c:when>
					<c:otherwise>
					...
					<td colspan="4">
						<ol class="paging">
						    ...
						    <c:choose>
						    	...
						    	<c:otherwise>
						    		<a href="${pageContext.request.contextPath}/MyController?cmd=list&cPage=${pvo.beginBlock-pvo.pagePerBlock}"><li>이전으로</li></a>
						    	</c:otherwise>
						    </c:choose>
						    <%-- 블록안에 들어간 페이지번호들 --%>
							<c:forEach begin="${pvo.beginBlock}" end="${pvo.endBlock}" step="1" var="k">
								...
								<c:choose>
									<c:when test="${k == pvo.nowPage}"> <%-- 같으면 현재 페이지 --%>
										<li class="now">${k}</li> <%-- 현재 페이지 --%>
									</c:when>
									<c:otherwise>
